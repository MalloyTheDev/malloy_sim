...
M1-M20 are complete: the math, time, sim-core, N-body, scenario, ASCII, collide,
...
rigid-body rotation, which is the first piece of 3D with no 2D form at all: a
body with three distinct principal moments tumbles under no torque, and in two
dimensions it cannot. Everything else is still 2D, which is most of the
...
M1-M20 are done. The classical mechanics track is complete, including the
...
and a 3D gravity world, and M20 added quaternions and torque-free 3D rotation.
Five finished domains sit behind multi-domain dispatch, across seven scenario
types. Work stays gated one milestone at a time; see
