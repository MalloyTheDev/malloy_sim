...
contact; M23 added friction, so a sliding sphere rolls. The rest is still 2D
and is the larger share of the project: sphere-sphere contacts, a translational
force on a 3D body, and the remaining domains. 3D is the destination rather than a possibility left
...
and under an applied torque, a sphere bouncing on a 3D ground plane, and
friction that makes a sliding sphere roll (milestones M1-M23).
...
**M23: 3D friction**). The project builds clean under
...
| `malloy_rigid` | STATIC | `RigidBody2D`, mass properties, pose integration, impulses, contact response, uniform gravity, friction; `RigidBody3D` and `Rigid3DWorld`, rotation free/torqued and sphere-plane contacts with restitution and friction |
...
and runnable with the shipped binary. 16 templates ship across 7 domains, and
...
- **the rest of 3D**. M19 shipped `Vec3` and 3D gravity, M20 quaternions and
  ...
  (a sphere on a ground plane), and M23 friction for it; sphere-sphere contacts,
  a general inertia TENSOR, a translational force on a 3D body, and 3D versions
  of the remaining domains are each their own milestone
  (`docs/decisions/0009-three-dimensions-are-the-destination.md`)
